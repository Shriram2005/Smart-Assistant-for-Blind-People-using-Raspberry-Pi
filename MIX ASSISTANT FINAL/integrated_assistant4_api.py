...
# Audio file paths and directory
AUDIO_DIR = "assistant_audio"
AUDIO_PATHS = {
    # OCR-related audio files
    'original': os.path.join(AUDIO_DIR, "original_audio.mp3"),
    ...
    'no_text': os.path.join(AUDIO_DIR, "no_text_found.mp3"),
    'complete': os.path.join(AUDIO_DIR, "translation_complete.mp3"),
    'ready': os.path.join(AUDIO_DIR, "ready_sound.mp3"),
    'error': os.path.join(AUDIO_DIR, "error_sound.mp3"),
    'stop': os.path.join(AUDIO_DIR, "stop_sound.mp3"),
    
    # Common responses
    'greeting': os.path.join(AUDIO_DIR, "greeting.mp3"),
    'goodbye': os.path.join(AUDIO_DIR, "goodbye.mp3"),
    'not_understood': os.path.join(AUDIO_DIR, "not_understood.mp3"),
    'thinking': os.path.join(AUDIO_DIR, "thinking.mp3"),
    'processing': os.path.join(AUDIO_DIR, "processing.mp3"),
    'success': os.path.join(AUDIO_DIR, "success.mp3"),
    'wait': os.path.join(AUDIO_DIR, "wait.mp3")
}

# Pre-defined feedback messages for audio files
FEEDBACK_MESSAGES = {
    # OCR system feedback
...
class SmartAssistant:
    def __init__(self, name="Assistant", voice_index=0):
        ...
        # Ensure audio directory exists and initialize audio files
    ...
    def initialize_audio_system(self):
        """Create audio directory and pre-generate all necessary audio files."""
        try:
            ...
            # Generate all pre-defined audio files
            for key, message in FEEDBACK_MESSAGES.items():
                audio_path = AUDIO_PATHS[key]
                if not os.path.exists(audio_path):
                    logger.info(f"Generating audio file: {key}")
            ...
            logger.error(f"Audio system initialization error: {str(e)}")
            return False
    ...
    def translate_text(self, text, target_lang):
        ...
        for attempt in range(max_retries):
            try:
                ...
                # Print the translation to the console for better accessibility
                target_lang_name = SUPPORTED_LANGUAGES.get(target_lang, {}).get('name', target_lang)
                print(f"\n--- Translation to {target_lang_name.capitalize()} ---")
    ...
    def capture_and_translate(self):
        ...
        try:
            ...
            if self.camera:
                ...
                # Create a thread-safe lock for updating translations
                translations_lock = threading.Lock()
                
                # Function to be executed by background translation threads
                def translate_in_background(target_code, target_info):
                    try:
                        ...
                        logger.error(f"Background translation to {target_info['name']} failed: {str(e)}")
                
                # Start translation threads for each target language
                translation_threads = []
                for target_code, target_info in SUPPORTED_LANGUAGES.items():
                    if target_code != detected_lang:
                        thread = threading.Thread(
                            target=translate_in_background,
                            args=(target_code, target_info)
                        )
                        thread.daemon = True  # Thread will exit when main program exits
                        translation_threads.append(thread)
                        thread.start()
                
                # Save to database in background if connection exists
                if self.connection_pool:
                    # Define database saving function for the background thread
                    def save_to_db_background():
                        try:
                            # Wait briefly for translations to be available
                            time.sleep(1)
                            # Get a thread-safe copy of the translations
                            with translations_lock:
                                translations_copy = self.last_translated_text.copy()
                            # Save to database
                            self.save_to_database(extracted_text, translations_copy)
                        except Exception as e:
                            logger.error(f"Background database save failed: {str(e)}")
                    
                    # Start database thread
                    db_thread = threading.Thread(target=save_to_db_background)
                ...
                # Note: Translation threads continue running in the background
                # and will be available when the user requests them
        ...
        try:
            ...
            with self.audio_lock:
                ...
                if self.stop_speaking:
                    ...
                    subprocess.run(["mpg123", "-q", AUDIO_PATHS['stop']])
    ...
    def check_for_stop_command(self):
        ...
        try:
            ...
            # Be resilient to errors in this background thread
            print(f"Error in check_for_stop_command: {e}")
            pass
    ...
    def process_query(self, query):
        ...
        if not query:
            self.play_audio(AUDIO_PATHS['not_understood'])
        ...
        # Time queries
        if any(phrase in query_lower for phrase in ["what is time", "current time", "time now", "what's the time", "what time is it"]):
            return self.get_current_time()
            
        # Date queries
        if any(phrase in query_lower for phrase in ["what date", "current date", "date today", "what's the date", "what day", "what's today", "today's date", "today date", "what is date"]):
        ...
        if any(word in query_lower for word in self.categories['greeting']):
            self.play_audio(AUDIO_PATHS['greeting'])
            return ""
    ...
    def run(self):
        ...
        try:
            # self.play_audio(AUDIO_PATHS['ready'])
            
            while self.is_active:
                query = self.listen()
                
                if query:
                    #if not any(phrase in query.lower() for phrase in ['stop', 'shut up', 'be quiet']):
                        # self.play_audio(AUDIO_PATHS['thinking'])
    ...
    def get_wikipedia_info(self, query):
        ...
        try:
            self.play_audio(AUDIO_PATHS['processing'])
...
if __name__ == "__main__":
    ...
    # Kill any audio that might be playing at program start
    try:
        subprocess.run(['pkill', '-f', 'mpg123'], stderr=subprocess.DEVNULL)
        time.sleep(0.5)
    except Exception as e:
        print(f"Note: Could not stop audio processes - {e}")

    # Check if audio directory exists, create it if it doesn't
    if not os.path.exists(AUDIO_DIR):
        os.makedirs(AUDIO_DIR)
