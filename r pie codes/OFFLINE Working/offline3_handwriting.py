...
gc.enable()
torch.set_num_threads(2)  # Limit torch threads
cudnn.benchmark = False   # Disable cudnn benchmark
cudnn.deterministic = True  # Use deterministic algorithms

# Limit OpenCV threads
cv2.setNumThreads(2)     # Limit OpenCV threads

def initialize_system():
    """Initialize camera, translator, EasyOCR and create feedback sounds."""
    global camera, translator, reader
    
    try:
        # Initialize EasyOCR with minimal memory settings
        ...
        logger.info("System initialized successfully with EasyOCR")
        return True
...
def classify_text(text):
    """Classify the type of text using basic rules."""
    # Simple rule-based classification
    if text.replace(' ', '').isdigit():
        return TEXT_CATEGORIES[1]
    
    # Check for address indicators
    address_keywords = ['street', 'road', 'avenue', 'lane', 'drive', 'boulevard']
    if any(keyword in text.lower() for keyword in address_keywords):
        return TEXT_CATEGORIES[2]
    
    # Check for name patterns (e.g., "Mr.", "Mrs.", "Dr.")
    name_prefixes = ['mr', 'mrs', 'ms', 'dr', 'prof']
    if any(text.lower().startswith(prefix) for prefix in name_prefixes):
        return TEXT_CATEGORIES[3]
...
def extract_text(image_path):
    ...
    try:
        # First try EasyOCR for handwriting with memory optimization
        try:
            # Resize image before processing to reduce memory usage
            img = cv2.imread(image_path)
            height, width = img.shape[:2]
            
            # Use smaller maximum dimension to reduce memory usage
            ...
            cv2.imwrite(resized_path, enhanced)
            
            # Clear variables to free memory
            del img, resized_img, gray_img, enhanced
            gc.collect()
            
            # Process with EasyOCR with minimal memory settings
            results = reader.readtext(
                resized_path,
                batch_size=1,
                paragraph=True,
                detail=0,  # Only get text results
                workers=1,  # Use single worker
                beamWidth=1  # Reduce beam width
            )
            ...
            if results:
                # Join results directly without storing intermediate lists
                handwritten_text = ' '.join(results)
                
                logger.info(f"EasyOCR detected text")
                category = classify_text(handwritten_text)
                logger.info(f"Text category: {category}")
                return handwritten_text, 'en'
        ...
        if highest_confidence >= threshold:
            ...
            if highest_confidence > max_other_confidence + 0.1:
                ...
                category = classify_text(best_lang[1]['text'])
                logger.info(f"Text category: {category}")
...
def capture_and_translate():
    ...
    try:
        # Configure and capture image with lower resolution
        camera.stop()
        config = camera.create_still_configuration(main={"size": (1920, 1080)})  # Lower resolution
        ...
        # Clear memory before heavy processing
        gc.collect()
