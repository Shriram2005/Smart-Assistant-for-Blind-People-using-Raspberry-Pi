...
def capture_and_translate(target_lang1='hi', target_lang2='mr'):
    ...
    try:
        # Translate to Hindi
        translated_text_1 = translator.translate(
            extracted_text, 
            src='en', dest=target_lang1
        ).text
        print(f"Translated Text ({target_lang1}): {translated_text_1}")
        tts_translated_1 = gTTS(translated_text_1, lang=target_lang1)
        tts_translated_1.save(AUDIO_PATHS['translated_1'])

        # Translate to Marathi
        translated_text_2 = translator.translate(
            extracted_text, 
            src='en', dest=target_lang2
        ).text
        print(f"Translated Text ({target_lang2}): {translated_text_2}")
        tts_translated_2 = gTTS(translated_text_2, lang=target_lang2)
        tts_translated_2.save(AUDIO_PATHS['translated_2'])
...
def handle_button_press():
    ...
    actions = {
        ...
        3: lambda: os.system(f"mpg123 {AUDIO_PATHS['translated_2']}")
    }
