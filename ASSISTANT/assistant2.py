...
class SmartAssistant:
    def __init__(self, name="Assistant", voice_index=0):
        ...
        self.wake_words = [
            self.name.lower(),  # Assistant's name
            "hey assistant",
            "okay assistant",
            "hello assistant",
            "hi assistant",
        ...
        # State tracking
        self.is_active = True
        self.is_listening = False  # Whether actively listening for commands
        self.last_wake_time = 0    # Timestamp of last wake word detection
        self.last_activity_time = 0  # Timestamp of last speech detected
        self.inactivity_timeout = 15  # Seconds of inactivity before auto-sleep
        ...
        self.speak(f"Hello, I'm {self.name}, your smart assistant. I'm ready to help. Just say one of my wake words to get started.")
            ...
            if topic.lower() in query_lower:
                if isinstance(topic_data, str):
                    ...
                    if "capital" in query_lower:
                        if "india states" in topic.lower():
    ...
    def listen_for_wake_word(self):
        ...
        print(f"{self.name} is waiting for a wake word...")
        ...
        with self.microphone as source:
            try:
                ...
                # Set a dynamic energy threshold
                self.recognizer.energy_threshold = 250
                self.recognizer.dynamic_energy_threshold = True
                
                # Listen for wake words
                while self.is_active and not self.is_listening:
                    try:
    ...
    def listen(self):
        ...
        if current_time - self.last_activity_time > self.inactivity_timeout:
            ...
            self.speak("I haven't heard anything for a while. Just say a wake word when you need me again.")
            return ""
        ...
        with self.microphone as source:
            try:
                ...
                try:
                    # Use a shorter timeout for better inactivity detection
                    audio = self.recognizer.listen(source, timeout=5, phrase_time_limit=15)
                    # Update activity timestamp on successful audio capture
                    self.last_activity_time = time.time()
                except sr.WaitTimeoutError:
                    # If we timeout waiting for speech, check our overall inactivity timer
                    current_time = time.time()
                    if current_time - self.last_activity_time > self.inactivity_timeout:
                        self.is_listening = False
                        if self.listening_sound:
                ...
                if text:
                    ...
                    # Update activity timestamp on successful speech recognition
                    self.last_activity_time = time.time()
                    
                    # Check if this is a stop listening command
                    if any(phrase in text.lower() for phrase in ["stop", "stop listening", "go to sleep", "that's all", "thank you"]):
                        self.is_listening = False
                        self.speak("I'll stop listening now. Just say a wake word when you need me again.")
                        return ""
                ...
                if self.listening_sound:
                    pygame.mixer.stop()
                # Check inactivity timeout after a timeout
                current_time = time.time()
                if current_time - self.last_activity_time > self.inactivity_timeout:
                    self.is_listening = False
                    self.speak("I haven't heard anything for a while. Just say a wake word when you need me again.")
    ...
    def get_wikipedia_info(self, query):
        """Get information from Wikipedia with enhanced reliability"""
        try:
            # Clean up the query
            search_query = query
            ...
            # First try direct Wikipedia API with better error handling
            try:
                # Search for Wikipedia pages
                search_results = wikipedia.search(search_query, results=5)  # Increase results to find better matches
                ...
                if search_results:
                    ...
                    for result in search_results:
                        try:
                            ...
                            # If summary seems too short, try to get more content
                            if len(summary) < 100 and page.content:
                                # Get first paragraph from content
                                first_para = page.content.split('\n\n')[0]
                                if len(first_para) > len(summary):
                                    return first_para
                    ...
                    # If all results failed, try fallback method
                    return None
                else:
                    return None
            except wikipedia.exceptions.DisambiguationError as e:
                # If there's a disambiguation, try each option
            ...
            # If direct page fails, try search page
            if response.status_code != 200:
                url = f"https://en.wikipedia.org/w/index.php?search={search_query.replace(' ', '+')}"
                response = requests.get(url, headers=headers, timeout=10)
            ...
            if response.status_code == 200:
                ...
                search_heading = soup.select_one('h1#firstHeading')
                if search_heading and 'Search results' in search_heading.text:
                    # Find the first search result and follow it
                    first_result = soup.select_one('div.mw-search-result-heading a')
                    if first_result and first_result.get('href'):
                        result_url = f"https://en.wikipedia.org{first_result.get('href')}"
                        response = requests.get(result_url, headers=headers, timeout=10)
                        if response.status_code == 200:
                            soup = BeautifulSoup(response.text, 'html.parser')
                
                # Get the first paragraph of content that's substantial
    ...
    def search_web(self, query):
        """Search the web for information with improved accuracy and robustness"""
        try:
            ...
            # Try multiple selectors for Google's answer boxes
            answer_selectors = [
                'div.Z0LcW', 'div.IZ6rdc', 'div.wDYxhc', 'div.kno-rdesc',
                'div.VwiC3b', 'span.hgKElc', 'div.uhHOwf', 'div.LGOjhe'
            ]
            
            for selector in answer_selectors:
                answer_div = soup.select_one(selector)
                if answer_div and len(answer_div.text.strip()) > 30:  # Only accept substantial answers
                    return answer_div.text.strip()
                
            # Look for search result snippets with better handling of nested content
            result_selectors = [
                'div.BNeawe s3v9rd AP7Wnd',  # Main snippets
                'div.kCrYT',                 # Alternate snippets
                'div.tF2Cxc',                # Standard result container
                'div.yuRUbf',                # Title container (with description nearby)
                'div.IsZvec',                # Description container
                'div.VwiC3b'                 # Another description class
            ]
            ...
            # Try alternate approach - look for any content with substantial text
            all_text_elements = soup.find_all(['p', 'div', 'span'], class_=True)
            valid_results = []
            
            for element in all_text_elements:
                text = element.text.strip()
                # Only consider elements with substantial, relevant text
                if len(text) > 100 and len(text) < 1000 and any(term in text.lower() for term in search_query.lower().split()):
                    valid_results.append((text, len(text)))
            
            # Sort by text length - moderate length texts are often most useful
            if valid_results:
                valid_results.sort(key=lambda x: -x[1])
                return valid_results[0][0]
            
            # If web search fails, try an alternative search engine (Bing)
            try:
                bing_url = f"https://www.bing.com/search?q={search_query.replace(' ', '+')}"
                bing_response = requests.get(bing_url, headers=headers, timeout=10)
                bing_soup = BeautifulSoup(bing_response.text, 'html.parser')
                
                # Look for Bing's answer snippets
                bing_answers = bing_soup.select('div.b_snippet') or bing_soup.select('div.b_caption')
                if bing_answers:
                    for answer in bing_answers:
                        if len(answer.text.strip()) > 50:
                            return answer.text.strip()
            except Exception as bing_error:
                if self.debug_mode:
                    print(f"Bing search error: {bing_error}")
            
            # If all web search approaches fail, return None
            return None
    ...
    def process_query(self, query):
        """Process the user's query and generate a more accurate response with priority on web search"""
        if not query:
        ...
        if category == "time":
        ...
        elif category == "web_search" or category in ["wikipedia", "general"]:
            # PRIORITY ROUTE: For web search and general categories, go straight to web methods
            ...
            if wiki_result:
                ...
                if search_result:
                    ...
                    # Fall back to "I couldn't find" response
                    response = f"I'm sorry, I couldn't find information about '{query}'. Could you please rephrase your question?"
        else:
            # For all other categories, try web sources first
    ...
    def run(self):
        ...
        try:
            while self.is_active:
                # First listen for wake word if we're not already listening
                if not self.is_listening:
                    self.listen_for_wake_word()
                    continue
                    
                # If we're actively listening, process commands
                query = self.listen()
                
                # Check for inactivity timeout
                current_time = time.time()
                if self.is_listening and (current_time - self.last_activity_time > self.inactivity_timeout):
                    self.is_listening = False
                    self.speak("I haven't heard anything for a while. Just say a wake word when you need me again.")
                    continue
                ...
                if query:
                    ...
                    self.speak(response)
                    # Update activity time after successful interaction
                    self.last_activity_time = time.time()
