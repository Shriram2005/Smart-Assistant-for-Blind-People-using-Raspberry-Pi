...
class SmartAssistant:
    def __init__(self):
        ...
        # Initialize EasyOCR reader (replace pytesseract)
        print("Initializing EasyOCR...")
        self.reader = easyocr.Reader(['en'])  # English only for speed
    ...
    def search_knowledge_base(self, query):
        ...
        # [Knowledge base search implementation remains the same]
        # This function is unchanged as it's not the performance bottleneck
        ...
        # Rest of the knowledge base search logic remains unchanged
        # [...]
    ...
    def translate_text(self, target_lang):
        ...
        try:
            ...
            if not self.last_extracted_text or self.last_extracted_text.strip() == "":
                ...
                # Read the text from the last captured image with EasyOCR
                print("No stored text found. Reading image...")
                results = self.reader.readtext(self.last_image_path)
                text = " ".join([result[1] for result in results])
                self.last_extracted_text = text
            else:
